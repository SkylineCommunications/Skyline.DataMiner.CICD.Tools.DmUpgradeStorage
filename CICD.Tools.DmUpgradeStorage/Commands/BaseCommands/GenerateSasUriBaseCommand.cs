namespace Skyline.DataMiner.CICD.Tools.DmUpgradeStorage.Commands.BaseCommands
{
    ...
    internal abstract class GenerateSasUriBaseCommand : BaseCommand
    {
        protected GenerateSasUriBaseCommand(string name, string? description = null) : base(name, description)
        {
            AddOption(new Option<IFileInfoIO>(
                aliases: ["--output-file", "-of"],
                description: "The output file which will contain the SAS URI(s).",
    ...
    internal abstract class GenerateSasUriBaseCommandHandler : BaseCommandHandler
    {
